...
import ManagerAssignedUsers from './ManagerAssignedUsers';
      ...
      <div className="h-full w-full flex flex-col overflow-hidden bg-transparent transition-all">
        <div className="flex-1 overflow-auto p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
              <h1
                className={`text-2xl ${isDarkMode ? 'text-zinc-200' : 'text-gray-900'} font-bold`}
              >
                Manage Managers
              </h1>
          ...
          {/* Table Container */}
          <div
            className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-zinc-800/50 border-zinc-700/50' : 'bg-white border-gray-200 shadow-lg shadow-gray-200/20'}`}
          >
            {loading ? (
              <div className="text-center py-16">
                <div
                  className={`inline-block animate-spin rounded-full h-10 w-10 border-b-2 ${isDarkMode ? 'border-cyan-400' : 'border-blue-500'}`}
                ></div>
                <p
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    ...
                    >
                      <th
                        className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider ${isDarkMode ? 'text-zinc-400' : 'text-gray-600'}`}
                      >
                        Name
                      </th>
                      <th
                      ...
                      <th
                        className={`px-6 py-3 text-center text-xs font-semibold uppercase tracking-wider ${isDarkMode ? 'text-zinc-400' : 'text-gray-600'}`}
                      >
                        Employees
                      </th>
                      <th
                  ...
                  <tbody
                    className={
                      isDarkMode ? 'divide-y divide-zinc-700/50' : 'divide-y divide-gray-200'
                    }
                  >
                    {managers.map((manager) => (
                      <tr
                        key={manager._id}
                        className={`transition-colors ${isDarkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-100'}`}
                      >
                        ...
                        <td
                          className={`px-6 py-4 whitespace-nowrap ${isDarkMode ? 'text-zinc-300' : 'text-gray-700'}`}
                        >
                          {manager.email}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          >
                            <div
                              className={`w-1.5 h-1.5 rounded-full ${manager.isRegistered ? 'bg-green-500' : 'bg-yellow-500'}`}
                            ></div>
                            {manager.isRegistered ? 'Registered' : 'Pending'}
                          </span>
                        </td>
                        <td
                          className={`px-6 py-4 whitespace-nowrap text-center ${isDarkMode ? 'text-zinc-300' : 'text-gray-700'}`}
                        >
                          <button
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              className={`p-2 rounded-md transition-colors ${isDarkMode ? 'text-zinc-400 hover:bg-zinc-700 hover:text-blue-400' : 'text-gray-500 hover:bg-gray-100 hover:text-blue-600'}`}
                              title="Edit"
                            >
                              <Edit size={16} />
                            </button>
