...
const CreateManagerModal = ({ isOpen, onClose, onManagerCreated }) => {
  ...
  const handleCreateManager = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to create manager.', { id: loadingToast });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className={`w-full max-w-md rounded-2xl shadow-2xl relative ${isDarkMode ? 'bg-zinc-900 border border-zinc-700' : 'bg-white'}`}>
        <div className={`p-6 border-b ${isDarkMode ? 'border-zinc-700' : 'border-gray-200'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Add New Manager</h2>
              <p className={`text-sm ${isDarkMode ? 'text-zinc-400' : 'text-gray-600'}`}>Invite a new manager to the company.</p>
            </div>
          </div>
        </div>
