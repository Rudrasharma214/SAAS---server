          ...
          <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-zinc-800/50 border-zinc-700/50' : 'bg-white border-gray-200 shadow-lg shadow-gray-200/20'}`}>
            {loading ? (
              <div className="text-center py-16">
                ...
                <p className={`mt-4 text-lg font-medium ${isDarkMode ? 'text-zinc-400' : 'text-gray-500'}`}>Loading employee...</p>
              </div>
            ) : users.length === 0 ? (
              <div className="text-center py-16">
                <UserPlus size={40} className={`mx-auto mb-4 ${isDarkMode ? 'text-zinc-600' : 'text-gray-400'}`} />
                <p className={`text-lg font-medium ${isDarkMode ? 'text-zinc-400' : 'text-gray-600'}`}>No employee found.</p>
                <p className={`mt-1 text-sm ${isDarkMode ? 'text-zinc-500' : 'text-gray-500'}`}>Get started by creating a new employee.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className={isDarkMode ? 'divide-y divide-zinc-700/50' : 'divide-y divide-gray-200'}>
                    {users.map((user) => (
                      <tr key={user._id} className={`transition-colors ${isDarkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-50'}`}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            ...
                            />
                            <span className={`font-medium ${isDarkMode ? 'text-zinc-200' : 'text-gray-900'}`}>{user.name}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${
                            ...
                            <div className={`w-1.5 h-1.5 rounded-full ${user.isRegistered ? 'bg-green-500' : 'bg-yellow-500'}`}></div>
